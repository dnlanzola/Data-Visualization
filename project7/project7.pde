...
int colCount;
float xHistogram1[], xHistogram2[], xHistogram3[], xHistogram4[];
float xMin1, xMax1, xMin2, xMax2, xMin3, xMax3, xMin4, xMax4;
float yMin1, yMax1, yMin2, yMax2, yMin3, yMax3, yMin4, yMax4;
float xAxisValuesAux, xPerAux;
int yAxisValuesAux, yPerAux;
int frequency;
int yHistogram0[], yHistogram1[], yHistogram2[], yHistogram3[], yHistogram4[];
float minX, maxX;
float minY, maxY;
float xAxisValues[];
...
String stdDev1Aux, stdDev2Aux, stdDev3Aux, stdDev4Aux;
...
void fileSelected(File selection) {
  if (selection == null) {
    ...
    myCorrgram = new Corrgram( myTable, myTable.getColumnTitles()[0] );
    myHistogram = new Histogram( myTable, myTable.getColumnTitles()[0] );
  }
}
...
void draw()
{
  ...
  if( myBarchart != null ){
    ...
    if( myLinechart != null ){
      ...
      if( myCorrgram != null ){
       myCorrgram.setPosition( 600, 200, width/2,150);
       myCorrgram.draw();
        }  
        
       if( myHistogram != null ){
       myHistogram.setPosition( 0, 0, width, height );
       myHistogram.draw();
        } 
  }
